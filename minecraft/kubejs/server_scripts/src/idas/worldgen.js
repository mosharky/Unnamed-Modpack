...
function idasWorldgen(e) {
    ...
    const idasSpawners = [
        ...
        const json = global.readJsonFileFromMod('data', 'idas', `idas:integrated_structure_spawners/${spawner}.json`)
        json.mobs.forEach(mob => {
            if (global.ENTITY_SWAPPER.get(`${mob.name}`) != undefined) {
                mob.name = global.ENTITY_SWAPPER.get(`${mob.name}`)
            }
        })
        e.addJson(`idas:integrated_structure_spawners/${spawner}.json`, json)
    })
    ...
    const idasStructures = [
        ...
        const json = global.readJsonFileFromMod('data', 'idas', `idas:worldgen/structure/${structure}.json`)
        ...
        e.addJson(`idas:worldgen/structure/${structure}.json`, json)
    })
}